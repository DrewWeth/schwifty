  ...
  function createWindow () {
    ...
    mainWindow = new BrowserWindow({width: 1280, height: 780, minWidth:800, minHeight:600, title: "Schwifty Text Analyzer"});
    ...
    function resolveContact(arg){
      var contactObj = contactLookup[cleanNumber(arg)]
      return { friendly: contactObj.First + " " + contactObj.Last, number: arg };
    }
    ...
    ipcMain.on("text-search-form-submitted", function(event, arg){
      console.log(arg);
      var matchingMessages = searchTexts(arg)
      console.log("searchTexts:");
      event.sender.send("text-search-results", matchingMessages);
    });
  ...
  function searchTexts(target){
    return allMessages.filter(function(message){
      return message.message.indexOf(target) >= 0;
    });
  }
